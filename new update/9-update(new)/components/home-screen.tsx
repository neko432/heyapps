...
function FloatingSymbols({ onDiscover }: { onDiscover: (message: string) => void }) {
  return (
    <div className="pointer-events-none fixed inset-0 z-0 hidden overflow-hidden md:block opacity-50 2xl:opacity-100">
      {FLOATING.map((f, i) => (
        <motion.button
          key={f.t}
          type="button"
...
export function HomeScreen({ onSelectCategory }: { onSelectCategory: (c: Category) => void }) {
  ...
  return (
    <div className="relative z-10 mx-auto flex min-h-svh w-full max-w-6xl flex-col px-5 py-8 md:px-8 xl:px-10">
      ...
      <AnimatePresence>
        {message && (
          <motion.div 
            ...
            transition={{ type: "spring", stiffness: 420, damping: 28, opacity: { duration: 0.2 } }} 
            className="overflow-hidden"
          >
            <div role="status" className="secret-message relative flex items-start gap-3 rounded-2xl border-2 border-primary/25 bg-card px-5 py-4 font-sans text-base font-bold leading-relaxed text-foreground shadow-pop-sm sm:text-lg">
      ...
      <footer className="mt-16 text-center text-[0.65rem] font-bold leading-relaxed text-muted-foreground/60 pb-8">
        <p>
          UI / Animation Inspired by:{" "}
          <a href="https://yui540.com/" target="_blank" rel="noopener noreferrer" className="underline hover:text-foreground">
            yui540
          </a>
        </p>
        <p className="mt-1">
          サウンドには
          <a href="https://on-jin.com/" target="_blank" rel="noopener noreferrer" className="underline mx-1 hover:text-foreground">On-Jin ～音人～</a>、
          <a href="https://maou.audio/" target="_blank" rel="noopener noreferrer" className="underline mx-1 hover:text-foreground">魔王魂</a>、
          <a href="https://soundeffect-lab.info/" target="_blank" rel="noopener noreferrer" className="underline mx-1 hover:text-foreground">効果音ラボ</a>
          の素材を使用しています。
        </p>
      </footer>
