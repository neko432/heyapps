...
export function GameScreen({ config, onFinish, onQuit }: Props) {
  ...
  // Mobile: onChange-based handler. On mobile the soft keyboard fires onChange
  // (sometimes without useful keyDown events). We diff against an internal
  // sentinel value to extract the newly typed character(s).
  ...
  function handleMobileInput(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (current.mode === "romaji") {
      // Free text typing for romaji mode on mobile
      commit(raw)
      const isMatch = raw === current.answerDisplay || toHiragana(raw) === current.reading || checkRomaji(current.reading, raw) === "match"
      if (isMatch) {
        celebrate()
    ...
    // Ascii mode logic (progressive typing)
    const prev = inputSentinel.current
    if (raw.length > prev.length) {
      const added = raw.slice(prev.length)
      for (const ch of added) {
        if (/^[a-zA-Z0-9+\-]$/.test(ch)) {
          evaluate(typedRef.current + ch.toLowerCase())
        }
      }
  ...
  return (
    ...
    >
      ...
      <input
        ...
        tabIndex={-1}
        className="absolute left-0 top-0 h-px w-px caret-transparent opacity-0"
        inputMode="text"
        enterKeyHint="send"
        autoComplete="off"
        autoCorrect="off"
