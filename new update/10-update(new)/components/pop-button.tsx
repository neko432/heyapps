...
export function PopButton({ variant = "primary", size = "md", className, children, onClick, onMouseEnter, ...props }: PopButtonProps) {
  const sizeCls =
    size === "lg" ? "px-8 py-4 text-lg" : size === "sm" ? "px-4 py-2 text-sm" : "px-6 py-3 text-base"
  const withShadow = variant !== "ghost"

  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    if (!props.disabled) sound.pop()
    if (onClick) onClick(e)
  }

  const handleHover = (e: React.MouseEvent<HTMLButtonElement>) => {
    if (!props.disabled) sound.hover()
